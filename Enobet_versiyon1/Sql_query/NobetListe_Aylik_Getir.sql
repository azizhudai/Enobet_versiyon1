...
select @jData=(NL.Data)
  from 
  NobetListeJson as NL where MONTH(JSON_VALUE(NL.Data, '$[0].NobetTarihi')) =@Month and JSON_VALUE(NL.Data, '$[0].MahalId')=@MahalId and Year(JSON_VALUE(NL.Data, '$[0].NobetTarihi'))=@Year;
  ...
  --CAST(DATEPART(d, NobetTarihi) AS VARCHAR) as GunSayi,
   FORMAT(cast(NobetTarihi as date),'d MMMM dddd yyyy') as TarihDetay
  ...
  from OPENJSON(@jData)WITH (KullaniciId int,
	   ...
	   order by cast(NobetTarihi as date) for json path)
	   select @result;
