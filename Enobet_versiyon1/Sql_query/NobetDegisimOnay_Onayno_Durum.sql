...
--set @Link='NjhIUzlRU1VCM1c0NkFPOQ=='; set @Durum=3;set @Eposta='mert.t1@dzkk.tsk';
set @NLJsonId=0;
set @Mesaj='';
set @AmirMi=0;
...
begin*/
	...
	set @SiradakiOnaylayacakPerId=@NobetKidemliId;
...
select NLTableList.KullaniciId,NLTableList.MahalId,NLTableList.NobetTarihi from OPENJSON(@jData)WITH (KullaniciId int,MahalId int,
	   ...
	   select @YeniTalepNobetId=Id from #TempNL T1 where KullaniciId=(select TalepEttigiPersonelId from NobetDegisimTalep T2 where T2.Link=@Link) and NobetTarihi=(select EskiTalepEdenKisininTarihi/*EskiTalepEttigiKisininTarihi */ from NobetDegisimTalep T2 where T2.Link=@Link)
	  ...
	  -- select @Mesaj;
	   If(OBJECT_ID('tempdb..#TempNL') Is Not Null)
		...
		if @Durum=3
		begin
		set @Mesaj='Nöbet Değişim Talebi Onay Makamı Tarafından Reddedilmiştir!';
		set @StateId=1;
		select @Mesaj Mesaj, @StateId StateId;
		end
		...
		update NobetDegisimTalep set 
	Durum=@Durum,
	[NobetKidemliOnayladiMi] = 1
      ,[KendiAmirOnayladiMi] = 1
      ,[TalepEttigiAmirOnayladiMi] = 1
      ,[OnayMakamiOnayladiMi] = @OnayMakamOnayMi
      ,[TalepEttigiPersonelOnayladiMi] = 1
	,[IslemYapanPersonelId]=@IslemYapanPersonelId
